...
export default function LinkSaverModal({
    setLinks,
    Links,
}: {
    ...
    // favicon fetcher
    const fetchFavicon = (link: string) => {
        try {
    ...
    const handleSave = () => {
        ...
        setLinks((prev) => {
            ...
            if (existing) {
                existing.links.push({ label, url, favicon });
                return [...prev];
            } else {
                return [
                    ...prev,
    ...
    const handleDeleteLink = (cat: string, linkIdx: number) => {
        setLinks((prev) =>
            prev.map((c) =>
                c.category === cat
